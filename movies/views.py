...
#     # Release seats after show time completed
#     old_shows = Booking.objects.filter(theater__time__lt=timezone.now())
#     for booking in old_shows:
#         booking.seat.is_booked = False
#         booking.seat.save()
#         booking.delete()
...
# def is_expired(self):
#     return not self.is_paid and self.reserved_at < timezone.now() - timedelta(minutes=5)

# Booking.is_expired = is_expired  # attach dynamically if not in model
...
#             'price_data': {
#                 'currency': 'inr',
#                 'product_data': {'name': theater.movie.name},
#                 'unit_amount': int(theater.price * 100),
#             },
#             'quantity': len(selected_seats),
#         }],
